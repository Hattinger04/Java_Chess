...
public class Rook extends Figur {
	...
	private final static int[] MOEGLICHE_MOVES_XY = { -8, -1, 1, 8};
	
	public Rook(int figurPosition, Farbe figurFarbe) {
		super (figurPosition, figurFarbe, FigurenTyp.ROOK, true);
	}
	
	public Rook(int figurPosition, Farbe figurFarbe, boolean isErsterMove) {
		super (figurPosition, figurFarbe, FigurenTyp.ROOK, isErsterMove);
	...
	public String toString() {
		return FigurenTyp.ROOK.toString(); 
	}
	

	private static boolean isAusnahmeErsteReihe(final int currentPosition, final int moeglichePosition) {
		return BoardUtils.ERSTE_SPALTE[currentPosition] && (moeglichePosition == -1);
	}

	private static boolean isAusnahmeAchteReihe(final int currentPosition, final int moeglichePosition) {
		return BoardUtils.ACHTE_SPALTE[currentPosition] && (moeglichePosition == 1);
	}
	

	@Override
	public Rook moveFigur(final Move move) {
		return new Rook(move.getPositionFeld(), move.getFigur().getFigurenFarbe());
	}

	@Override
	public int locationBonus() {
		return this.figurFarbe.rookBonus(this.figurPosition); 
	}
